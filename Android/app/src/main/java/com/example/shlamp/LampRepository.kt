...
internal class LampRepository(context: Context) {
                ...
                for (index in 0 until array.length()) {
                    ...
                    add(
                        LampDevice(
                            ...
                            routePreference = json.optEnum("routePreference", LampRoutePreference.AUTO),
                            powerMode = json.optEnum("powerMode", LampPowerMode.BALANCED),
                            runtimeState = json.optEnum("runtimeState", LampRuntimeState.UNKNOWN),
        ...
        lamps.distinctBy { normalizeId(it.lampId) }.forEach { lamp ->
            array.put(
                JSONObject().apply {
                    ...
                    put("routePreference", lamp.routePreference.name)
                    put("powerMode", lamp.powerMode.name)
                    put("runtimeState", lamp.runtimeState.name)
                    put("isOn", lamp.isOn)
    ...
    private fun mergePersistedRecords(
        ...
        return LampDevice(
            ...
            routePreference = exact?.routePreference
                ?: ordered.firstOrNull()?.routePreference
                ?: LampRoutePreference.AUTO,
            powerMode = ordered.firstOrNull { it.powerMode != LampPowerMode.BALANCED }?.powerMode
                ?: freshestState.powerMode,
            runtimeState = freshestState.runtimeState,
    ...
    private inline fun <reified T : Enum<T>> JSONObject.optEnum(key: String, fallback: T): T =
        optString(key, fallback.name).trim().takeIf(String::isNotBlank)?.let { raw ->
