...
internal enum class LampRoutePreference {
    AUTO,
    WIFI,
    BLUETOOTH,
    REMOTE
...
internal enum class LampPowerMode {
    BALANCED,
    MAX_BACKUP,
    BLE_ONLY,
    TOUCH_ONLY;

    val firmwareValue: String
        get() = when (this) {
            BALANCED -> "balanced"
    ...
    val binaryValue: Byte
        get() = ordinal.toByte()
}

internal enum class LampRuntimeState {
    ACTIVE,
    LAMP_ON_IDLE,
    OFF_RECENT,
    OFF_LONG,
...
internal data class LampDevice(
    ...
    val routePreference: LampRoutePreference = LampRoutePreference.AUTO,
