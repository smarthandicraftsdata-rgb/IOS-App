...
internal class LampConnectionManager(
    ...
    // Lamps selected during Add Lamp remain transient until the user presses the
    // explicit final Add/Finish button. This prevents a tap on a scan result from
    // silently creating a device card.
    private val transientLampIds = mutableSetOf<String>()

    // A cloud-only lamp may advertise a different local BLE identity. During a
    // known-device scan we briefly connect only to read FFE3, then attach the BLE
    // route when the firmware-reported cloud ID belongs to this account.
    private var knownDeviceScan = false
    private val identityProbeQueue = ArrayDeque<NearbyLamp>()
    private val queuedProbeAddresses = mutableSetOf<String>()
    private var identityProbeAddress: String? = null
    ...
    /** Immediately removes stale Bluetooth routes when Android reports that the radio is off. */
    fun onPhoneBluetoothDisabled() {
        bleManager.disconnect()
        _connecting.value = false
        var changed = false
        val currentNetworkKey = wifiController.currentWifiNetworkKey()
        _lamps.value = _lamps.value.map { lamp ->
            if (lamp.route != LampConnectionRoute.BLUETOOTH) {
                lamp
            } else {
                changed = true
                val savedNetworkKey = wifiNetworkKeyByLampId[normalizedLampId(lamp.lampId)]
                lamp.copy(
                    route = if (currentNetworkKey != null && savedNetworkKey == currentNetworkKey &&
                        !lamp.ipAddress.isNullOrBlank()
                    ) LampConnectionRoute.WIFI else LampConnectionRoute.OFFLINE
                )
            }
        }.sortedBy { it.name.lowercase() }
        if (changed) {
            _status.value = "Bluetooth is off; using another available connection"
        }
    }
    ...
    private fun mergeAliasIntoFull(
        ...
        return (exact ?: alias).copy(
            ...
            routePreference = exact?.routePreference ?: alias.routePreference,
            powerMode = if (aliasIsNearby) alias.powerMode else exact?.powerMode ?: alias.powerMode,
            runtimeState = if (aliasIsNearby) alias.runtimeState else exact?.runtimeState ?: alias.runtimeState,
    ...
    fun discoverKnownBluetoothLamps() {
        ...
        knownDeviceScan = true
        identityProbeQueue.clear()
        queuedProbeAddresses.clear()
        identityProbeAddress = null
    ...
    fun addLamp(nearby: NearbyLamp) {
        ...
        if (existing == null) transientLampIds.add(normalizedLampId(lamp.lampId))
        upsertLamp(lamp, persist = false)
    ...
    fun commitSelectedLamp(name: String, room: String) {
        val lamp = selectedLamp() ?: return
        val cleanName = name.trim().take(32).ifBlank { lamp.name.ifBlank { "SH Lamp" } }
        val cleanRoom = room.trim().take(24).ifBlank { "Unassigned" }
        transientLampIds.remove(normalizedLampId(lamp.lampId))
        upsertLamp(lamp.copy(name = cleanName, room = cleanRoom), persist = true)
    }

    fun setRoutePreference(lampId: String, preference: LampRoutePreference) {
        val lamp = localLamp(lampId) ?: return
        val updatedRoute = when (preference) {
            LampRoutePreference.BLUETOOTH -> if (bleManager.isConnectedTo(lamp.bleAddress)) {
                LampConnectionRoute.BLUETOOTH
            } else {
                lamp.route
            }
            LampRoutePreference.WIFI -> if (lamp.route == LampConnectionRoute.WIFI) {
        ...
        upsertLamp(lamp.copy(routePreference = preference, route = updatedRoute), persist = true)
        if (preference == LampRoutePreference.BLUETOOTH && !bleManager.isConnectedTo(lamp.bleAddress)) {
    ...
    fun powerModeLamp(
        ...
        val limitedBrightness = if (mode == LampPowerMode.MAX_BACKUP) {
            lamp.brightness.coerceAtMost(70)
        } else {
            lamp.brightness
        }
        upsertLamp(
            lamp.copy(
                powerMode = mode,
                runtimeState = if (mode == LampPowerMode.TOUCH_ONLY) {
                    LampRuntimeState.TOUCH_ONLY
                } else {
                    LampRuntimeState.ACTIVE
                },
                brightness = limitedBrightness,
                lastNonZeroBrightness = lamp.lastNonZeroBrightness.coerceAtMost(
                    if (mode == LampPowerMode.MAX_BACKUP) 70 else 100
                )
            ),
            persist = mode == LampPowerMode.BALANCED || mode == LampPowerMode.MAX_BACKUP
        )
        routeLampWifiThenBle(
            lampId = lamp.lampId,
            wifiAction = { host, callback -> wifiController.sendPowerMode(host, mode, callback) },
            bleAction = { bleManager.powerMode(mode) },
            onHandled = onHandled,
            onUnavailable = onUnavailable
        )
    }
    ...
    private fun routeLampWifiThenBle(
        ...
        if (lamp.routePreference == LampRoutePreference.REMOTE) {
            onUnavailable()
            return
        }
        if (lamp.routePreference == LampRoutePreference.BLUETOOTH) {
            if (bleManager.isConnectedTo(lamp.bleAddress)) {
                bleAction()
                ...
                reconnectLamp(lamp.lampId)
                _status.value = "Connecting to ${lamp.name} through Bluetooth…"
                onUnavailable()
            }
            return
        }
    ...
    private fun queueIdentityProbe(nearby: NearbyLamp) {
        if (!knownDeviceScan || nearby.bleAddress in queuedProbeAddresses) return
        val needsBluetoothIdentity = _lamps.value.any { lamp ->
            lamp.remoteLampId != null && lamp.bleAddress.isNullOrBlank()
        }
        if (!needsBluetoothIdentity) return
        queuedProbeAddresses.add(nearby.bleAddress)
        identityProbeQueue.addLast(nearby)
    }

    private fun beginNextIdentityProbe() {
        if (!knownDeviceScan || identityProbeAddress != null || _connecting.value) return
        while (identityProbeQueue.isNotEmpty()) {
            val nearby = identityProbeQueue.removeFirst()
            if (_lamps.value.any { it.bleAddress.equals(nearby.bleAddress, ignoreCase = true) }) {
                continue
            }
            identityProbeAddress = nearby.bleAddress
            identityProbeMatched = false
            _connecting.value = true
            _status.value = "Checking nearby lamp identity…"
            bleManager.connect(nearby.bleAddress, nearby.advertisedName)
            return
        }
    }
    ...
    override fun onScanUpdated(lamps: List<NearbyLamp>) {
        ...
        val selectedId = _selectedLampId.value?.let(::normalizedLampId)
        val ordered = lamps.sortedWith(
            compareByDescending<NearbyLamp> { nearby ->
                val resolved = resolveKnownLampId(nearby.lampId)
                selectedId != null && normalizedLampId(resolved) == selectedId
            }.thenByDescending { it.rssi }
        )

        var knownConnectionStarted = false
        ordered.forEach { nearby ->
            val localId = normalizedLampId(nearby.lampId)
            val reportedCloud = repository.reportedCloudIdFor(localId)
                ?.let(::normalizedLampId)
            if (reportedCloud != null && reportedCloud in knownCloudLampIds) {
                repository.linkLampIds(localId, reportedCloud)
            }

            val resolvedNearbyId = resolveKnownLampId(localId)
            val existing = _lamps.value.firstOrNull { known ->
                known.lampId.equals(resolvedNearbyId, ignoreCase = true) ||
                    known.lampId.equals(localId, ignoreCase = true) ||
                    known.bleAddress.equals(nearby.bleAddress, ignoreCase = true) ||
                    known.remoteLampId.equals(reportedCloud, ignoreCase = true)
            }

            if (existing == null) {
                queueIdentityProbe(nearby)
                return@forEach
            }
            ...
            val selected = selectedId == normalizedLampId(updated.lampId)
            val shouldPreferBluetooth = updated.routePreference == LampRoutePreference.BLUETOOTH
            val shouldRecoverCloudOnly = updated.route == LampConnectionRoute.OFFLINE &&
                updated.remoteLampId != null
            if (!knownConnectionStarted &&
                (selected || shouldPreferBluetooth || shouldRecoverCloudOnly) &&
                !bleManager.isConnectedTo(updated.bleAddress) &&
                !_connecting.value
            ) {
                knownConnectionStarted = true
                _connecting.value = true
                bleManager.connect(nearby.bleAddress, updated.bleName ?: updated.name)
            }
        }

        if (!knownConnectionStarted) beginNextIdentityProbe()
    }
    ...
    override fun onBleIdentity(identity: BleLampIdentity) {
        ...
        if (cloudId != null && cloudId.startsWith("SH-") && cloudId != localId) {
            ...
            if (cloudId in knownCloudLampIds) {
                repository.linkLampIds(localId, cloudId)
                rememberCloudLampIds(listOf(cloudId))
                identityProbeMatched = identityProbeAddress != null
                if (normalizedLampId(localId) in transientLampIds) {
                    transientLampIds.remove(normalizedLampId(localId))
                    transientLampIds.add(cloudId)
                }
                _status.value = "Known lamp verified through Bluetooth"
            } else {
    ...
    override fun onBleReady(lampId: String, address: String, advertisedName: String) {
        _connecting.value = false
        val wasIdentityProbe = identityProbeAddress.equals(address, ignoreCase = true)
        if (wasIdentityProbe && !identityProbeMatched) {
            identityProbeAddress = null
            bleManager.disconnect()
            handler.postDelayed(::beginNextIdentityProbe, 180L)
            return
        }
        ...
        val merged = (old ?: LampDevice(lampId = resolvedLampId, name = advertisedName)).copy(
            ...
            route = if (old?.route == LampConnectionRoute.WIFI &&
                old.routePreference != LampRoutePreference.BLUETOOTH
            ) {
                LampConnectionRoute.WIFI
            } else {
                LampConnectionRoute.BLUETOOTH
    ...
    override fun onBleDisconnected(address: String?) {
        _connecting.value = false
        if (identityProbeAddress.equals(address, ignoreCase = true)) {
            identityProbeAddress = null
            identityProbeMatched = false
            handler.postDelayed(::beginNextIdentityProbe, 180L)
        }
    ...
    override fun onBleLampStatus(status: BleLampStatus) {
        ...
        val updated = existing.copy(
            ...
            route = if (existing.route == LampConnectionRoute.WIFI &&
                existing.routePreference != LampRoutePreference.BLUETOOTH
            ) {
                LampConnectionRoute.WIFI
            } else {
                LampConnectionRoute.BLUETOOTH
    ...
    override fun onBlePowerMode(mode: LampPowerMode) {
        val lamp = selectedLamp() ?: return
        val limitedBrightness = if (mode == LampPowerMode.MAX_BACKUP) {
            lamp.brightness.coerceAtMost(70)
        } else {
        ...
        _status.value = "Power mode: ${mode.name.replace('_', ' ').lowercase().replaceFirstChar { it.uppercase() }}"
    }
    ...
    private fun persist() {
        repository.saveLamps(
            _lamps.value.filterNot { normalizedLampId(it.lampId) in transientLampIds }
        )
    }
}
