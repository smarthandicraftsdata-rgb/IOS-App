...
class AddLampActivity : ComponentActivity() {
    companion object {
        const val EXTRA_EXISTING_LAMP_ID = "existing_lamp_id"
        const val EXTRA_START_REMOTE_ACCESS = "start_remote_access"
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val existingLampId = intent.getStringExtra(EXTRA_EXISTING_LAMP_ID).orEmpty()
            .trim().uppercase(Locale.US)
        val startRemoteAccess = intent.getBooleanExtra(EXTRA_START_REMOTE_ACCESS, false)
        val initialDraft = if (startRemoteAccess && existingLampId.isNotBlank()) {
            val existing = manager.localLamp(existingLampId)
            LampSetupDraft(
                step = LampSetupStep.ENTER_CODE,
                lampId = existing?.lampId ?: existingLampId,
                cloudLampId = existing?.remoteLampId.orEmpty(),
                bleAddress = existing?.bleAddress.orEmpty(),
                advertisedName = existing?.bleName.orEmpty(),
                mode = LampSetupMode.REMOTE_ONLY,
                lampName = existing?.name.orEmpty(),
                roomName = existing?.room?.takeUnless { it == "Unassigned" }.orEmpty()
            )
        } else {
            setupStore.load()
        }
        ...
        setContent {
            SHLAMPTheme {
                AddLampWizard(
                    ...
                    showResumePrompt = !startRemoteAccess && initialDraft != null,
    ...
    private fun completeSetup(draft: LampSetupDraft, callback: (Result<String>) -> Unit) {
        ...
        manager.commitSelectedLamp(cleanLampName, cleanRoomName)

        if (draft.mode != LampSetupMode.REMOTE_ONLY) {
            ...
            if (Regex("SH-[A-Z0-9]{4,16}").matches(reportedCloudId) &&
                ...
                // Keep the manufacturing identity as a hint without silently
                // claiming remote ownership. Remote access is a separate opt-in.
                manager.recordReportedIdentity(localLampId, reportedCloudId)
            }
            setupStore.clear()
            val message = if (draft.mode == LampSetupMode.WIFI) {
                "Your lamp is ready on local Wi-Fi. Remote access can be enabled now or later."
            } else {
                "Your lamp is ready for nearby Bluetooth control. Remote access can be enabled later."
            }
...
private fun AddLampWizard(
    ...
    var showResume by remember { mutableStateOf(showResumePrompt && savedDraft != null) }
    ...
    LaunchedEffect(Unit) {
        if (!showResume && draft.step == LampSetupStep.DISCOVER) {
            onStartDiscovery { granted ->
                if (!granted) error = "Nearby device permission is needed to find your lamp."
            }
        } else if (!showResume && draft.mode == LampSetupMode.REMOTE_ONLY) {
            draft.lampId.takeIf(String::isNotBlank)?.let {
                manager.selectLamp(it, connectBle = false)
            }
        }
    }
        ...
        LazyColumn(
            ...
            item {
                WizardHeader(
                    ...
                    onBack = {
                        if (draft.mode == LampSetupMode.REMOTE_ONLY &&
                            draft.step == LampSetupStep.ENTER_CODE
                        ) {
                            onBack()
                        } else {
                            when (draft.step) {
                                ...
                                LampSetupStep.ENTER_CODE -> save(
                                    draft.copy(
                                        step = when (draft.mode) {
                                            null -> LampSetupStep.DISCOVER
                                ...
                                LampSetupStep.NAME -> save(
                                    draft.copy(
                                        step = when (draft.mode) {
                                            LampSetupMode.WIFI -> LampSetupStep.WIFI
                                            LampSetupMode.REMOTE_ONLY -> LampSetupStep.ENTER_CODE
                                            else -> LampSetupStep.CHOOSE_CONNECTION
                                        }
                                    )
                                )
                ...
                when (draft.step) {
                    ...
                    LampSetupStep.ENTER_CODE -> item {
                        SetupCard {
                            ...
                            Text(
                                if (draft.mode == LampSetupMode.REMOTE_ONLY) {
                                    "Enter the Lamp ID and claim code to enable control while away from home."
                                } else {
                                    "Scan the code on the lamp or packaging. Remote claiming stays optional."
                                },
                                color = SHLampDesign.TextSecondary
                            )
                            Spacer(Modifier.height(16.dp))
                                        ...
                                        result.onSuccess { payload ->
                                            ...
                                            save(
                                                draft.copy(
                                                    ...
                                                    claimCode = payload.claimCode,
                                                    step = when (draft.mode) {
                                                        LampSetupMode.REMOTE_ONLY -> LampSetupStep.NAME
                                                        LampSetupMode.WIFI -> LampSetupStep.WIFI
                                                        else -> LampSetupStep.DISCOVER
                                                    }
                                                )
                                            )
                            ...
                            Text(
                                if (draft.mode == LampSetupMode.REMOTE_ONLY) {
                                    "The claim code is required to enable remote access for a new lamp."
                                } else {
                                    "Remote access is optional and can be added after local setup."
                                },
                                color = SHLampDesign.TextSecondary,
                                style = MaterialTheme.typography.bodySmall
                            )
                            ...
                            Button(
                                onClick = {
                                    ...
                                    } else if (draft.mode == LampSetupMode.REMOTE_ONLY && manualClaimCode.isBlank()) {
                                        error = "Enter the claim code printed with the lamp."
                                    } else {
                                        error = ""
                                        save(
                                            draft.copy(
                    ...
                    LampSetupStep.NAME -> item {
                        SetupCard {
                            ...
                            Spacer(Modifier.height(18.dp))
                            Text(
                                if (draft.mode == LampSetupMode.REMOTE_ONLY) "Confirm lamp details" else "Name your lamp",
                                fontSize = 24.sp,
                            ...
                            Button(
                                ...
                                Text(
                                    if (finishing) "Finishing…" else if (draft.mode == LampSetupMode.REMOTE_ONLY) "Enable remote access" else "Add lamp",
                                    fontWeight = FontWeight.Bold
                                )
                            }
                        }
                    ...
                    LampSetupStep.READY -> item {
                        SetupCard {
                            ...
                            if (draft.mode != LampSetupMode.REMOTE_ONLY) {
                                OutlinedButton(
                                    onClick = {
                                        val lampId = manager.selectedLamp()?.lampId ?: draft.lampId
                                        context.startActivity(
                                            Intent(context, AddLampActivity::class.java)
                                    ...
                                    modifier = Modifier.fillMaxWidth(),
                                    border = BorderStroke(1.dp, SHLampDesign.Primary)
