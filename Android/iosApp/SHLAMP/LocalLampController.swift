...
final class LocalLampController: NSObject {
    ...
    func readStatus(host: String) async throws -> WiFiLampSnapshot {
        try await snapshot(from: try await request(host: host, path: "/api/status"), host: normalizedHost(host))
    }
    ...
    func sendBrightness(host: String, percent: Int) async throws -> WiFiLampSnapshot {
        ...
        return try await verified(host: host, path: "/api/brightness?value=\(value)") { $0.targetBrightness == value }
    }
    ...
    private func request(host: String, path: String) async throws -> Data {
        ...
        request.setValue("SHLAMP-iOS/1.3.1", forHTTPHeaderField: "User-Agent")
