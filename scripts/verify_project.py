...
BACKEND_HASHES = ROOT / "docs" / "BACKEND_BASELINE_SHA256.json"
...
try:
    ...
    if icon.size != (1024, 1024):
        errors.append(f"App icon must be 1024x1024, got {icon.size}")
    if icon.mode not in {"RGB", "P"}:
        errors.append(f"App icon should not contain alpha; mode is {icon.mode}")
except ModuleNotFoundError:
    pass
except Exception as exc:
    errors.append(f"Could not validate app icon: {exc}")
...
try:
    expected_hashes = json.loads(BACKEND_HASHES.read_text())
    for name, expected_hash in expected_hashes.items():
        path = IOS / name
        actual = hashlib.sha256(path.read_bytes()).hexdigest()
        if actual != expected_hash:
            errors.append(f"Backend integrity mismatch: {name}")
except Exception as exc:
    errors.append(f"Backend hash manifest is invalid: {exc}")
...
for required_ui in ["LampGridCell", "uiCanAttemptBasicControl", "uiSupportsNearbyControls", "BrandLogoView", "LampHeroRingView"]:
    if required_ui not in ui_text:
        errors.append(f"Required audited UI component missing: {required_ui}")
